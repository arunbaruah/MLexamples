...
import pandas as pd
# Read the file
f = pd.read_csv('StudentDataset.csv')
...
X_train_new = X_train[['Hours', 'sHours']]
X_test_new = X_test[['Hours', 'sHours']]

lr_new = LinearRegression()
lr_new.fit(X_train_new, Y_train)

y_predict_new = lr_new.predict(X_test_new)

# Calculate the RMSE with reduced features
rmse_new = math.sqrt(mean_squared_error(Y_test, y_predict_new))
